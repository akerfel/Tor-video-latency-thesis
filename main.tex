...
\usepackage{hyperref}   % clickable ToB and sources
\hypersetup{
    colorlinks,
    hidelinks,
    allcolors=black
...
\begin{figure}[!htb]
    ...
    \caption{An illustration of how a Tor client connects to a server.}
...
\[E_{max} = log_{2}(N)\]
Assume an adversary somehow is able to exclude a set of possible senders of a certain message, leaving a set \emph{S} where every user has the same probability of being the sender. The entropy for this set of users can be calculated as follows:
...
All test data can be found on GitHub \parencite{thesisGithub}. In appendix \ref{section:sampleTestData} we have visualized some of this test data. Each graph in the appendix represents one of the six tests conducted for each scenario. The graphs show how the latency relative to Zoom alternated between high and low values.
...
\begin{figure}[!htb]
    ...
    \caption{50th percentile (median) latency for each scenario. Blue represents median latency for the UDP scenario, relative to Zoom. Red represents latency relative to the UDP scenario's median latency. The black numbers show latency relative to Zoom.}
...
\begin{figure}[!htb]
    ...
    \caption{75th percentile latency for each scenario. Blue represents the 75th percentile latency for the UDP scenario, relative to Zoom. Red represents latency relative to the UDP scenario's 75th percentile latency. The black numbers show latency relative to Zoom.}
    \label{fig:75thPercentile}
\end{figure}

\subsection{The 95th percentile}
For the 95th percentile, the TCP scenario had very similar latency to the UDP scenario, the Sweden and Germany scenarios were about 350 ms behind (similar to lower percentiles), and the Global scenario was a whole second behind (much bigger difference compared to lower percentiles). 

\begin{figure}[!htb]
    \centering
    \includesvg[width=350pt]{resources/graphs/95th percentile relative latency.svg}
    \caption{95th percentile latency for each scenario. Blue represents the 95th percentile latency for the UDP scenario, relative to Zoom. Red represents latency relative to the UDP scenario's 95th percentile latency. The black numbers show latency relative to Zoom.}
...
\begin{figure}[!htb]
    ...
    \caption{99th percentile latency for each scenario. Blue represents the 99th percentile latency for the UDP scenario, relative to Zoom. Red represents latency relative to the UDP scenario's 99th percentile latency. The black numbers show latency relative to Zoom.}
...
\subsection{Degree of anonymity}
These values rely on the assumptions specified in section \ref{section:shannonentropy}. Vanilla Tor users connect to Tor relays from the entire world, which means that if no attacks have been made, the degree of anonymity is 100 \% in that system. This is because, in that scenario, adversaries can not exclude any users from the set of possible users by looking at the region of the relays that the user is connected to. For German Tor users the degree of anonymity is reduced to 82.6 \%; and for Swedish Tor users the degree of anonymity is reduced to 65.3 \%.
...
The UDP scenario was usually about 100 ms behind Zoom, with some latency spikes at \char`\~500 ms. For the 25th, 50th, and 75th percentiles, when comparing the scenarios to the UDP scenario, the TCP scenario was \char`\~200 ms behind, the Sweden and Germany scenarios about \char`\~300 ms, and the Global scenario \char`\~500 ms. For the 95th percentile, the TCP scenario was only slightly behind the UDP scenario, the Sweden and Germany scenarios were again only about 300 ms behind, however the Global scenario was a whole second behind. Finally, for the 99th percentile, the TCP scenario was again only slightly behind the UDP scenario, the Sweden scenario had the biggest relative latency of more than three seconds, the Germany scenario at one second, and the Global scenario at two seconds.

\break

From these results some key findings can be established, when comparing each scenario to the UDP scenario:
...
\begin{itemize}
  ...
  \vspace{-0.2cm}\item At the 95th percentile, the Global scenario had a much higher relative latency than all other scenarios, at more than 1000 ms.
...
The UDP scenario had a relative latency of 116 ms, meaning qTox lagged behind the Zoom-stream by 116 ms. It also had some latency spikes of over 500 ms in the 95th and 99th percentiles. This was unexpectedly high, since the UDP scenario used default qTox settings (except for IPv6 being turned off). One possible explanation for this result is that the underlying algorithms (e.g. compression) responsible for sending and receiving video could differ for qTox and Zoom. Furthermore, even though the TCP scenario usually performed worse than the UDP scenario, their latency spikes were similar, at about half a second relative to Zoom.

\subsection{TCP effect on latency}
Using TCP instead of UDP meant an increase of about 200 ms in latency. Since Tor only supports TCP, and the Global scenario added 517 ms compared to the UDP scenario, we can be quite certain that a large proportion (at least 200 ms, or 39 \%) of the latency induced by Tor can be attributed to the usage of TCP instead of UDP. This could mean that if one wants to send live video over Tor with low latency, it is equally important to effectively circumvent the inherited problems of TCP, as it is to change the path selection algorithm for Tor.
...
It is clear from our results that when limiting relays to the geographical location of the user, a significant amount of anonymity will be lost in several metrics. The entire point of using Tor to begin with is to increase anonymity, thus regionally limiting relays could seem counterproductive. However, it is important to note that our calculations assumed that the adversary was aware of the fact that the user was using the alternative path selection. If that is not the case, the anonymity will not be as negatively affected.

Moreover, our calculations for the probability of a compromised circuit and for the Shannon entropy rely on different assumptions. For the compromised circuit probability, it is enough that a specific adversary sets up relays in a specific region, and that at least \emph{one} user in that region limits their relay selection by region. Already at that point, the risk of the user using a compromised circuit will be greatly increased. But for the Shannon entropy calculation (and degree of anonymity, which relies on Shannon entropy), we assumed that \emph{all} users in the relevant region only used relays from that region, and that no one else used those relays. This is not the case at the moment, which means that if only a few people started using this type of path selection, the Shannon entropy and degree of anonymity metrics would not be as drastically affected.
...
We have made multiple assumptions and simplified our anonymity calculations, which has been specified in section \ref{section:anonymityMetrics}. Furthermore, we only calculated three different anonymity metrics. Regionally limiting the usage of Tor relays might have other unwanted effects on privacy, such as making deanonymization attacks more effective. This remains to be explored. Thus we do not recommend regionally limiting the relays with our methods.

\section{Sources of error}
One possible source of data error is the fact that the sender had a relatively low download speed of 13.11 Mbit/s, and a high ping of 43.81 ms. If these metrics had been more favorable, the results might have been different, especially considering that \emph{two} video streams were sent at the same time. Furthermore, each of the five scenarios only had about 600 data points each. With more data points the results would be more accurate, especially if more Tor circuits were tested (we tested six circuits for each scenario involving Tor). 
...
Furthermore, most other selection path studies managed to decrease latency, and in some cases also increased anonymity. An example of this is the CLAPS system, see section \ref{section:CLAPS}. Our method did indeed decrease latency, but also decreased anonymity in several metrics, and much more so than other studies. The severe decrease in anonymity is the weak point in our method.

\section{Further Studies}
It would be interesting to compare the Tor latency results with the latency induced when video calling using a VPN connection. VPN offers much lower latency than Tor for multiple reasons. First, VPN uses one proxy hop instead of three. Secondly, the VPN proxy is usually closer to the user than the Tor relays are. Lastly, VPN is not limited to sending TCP streams. However the anonymity for VPN is generally lower than for Tor. In a similar manner, the latency induced and anonymity when using Signal could also be examined.

As mentioned in section \ref{section:anonymityMetrics}, we did not attempt to estimate a realistic number of malicious relays, since it is a very complex task. Further studies could try to estimate the amount of malicious relays specifically for certain geographical regions, such as Sweden or Germany.
...
Multiple other scenarios could be interesting to explore. First, the effect of limiting relays by region has on download and upload speed (i.e. throughput). Secondly, the latency effect when several people are in the same group video call could be examined. qTox does not currently support multiple people video calls, but perhaps some other application could be used (it would need to allow for TCP to be used for video transfer, and the use of a SOCKS proxy). Thirdly, the relays could be limited to other countries than only Sweden and Germany.

Furthermore, in our scenarios we limited \emph{all} relays in the circuit regionally, but if only the guard or the exit was regionally limited, perhaps higher anonymity could be achieved (although the latency would be increased). Multiple other relay configurations could be tested, for example by using relays from a few countries close to the user, since that could partially circumvent relay-placement attacks. Regarding anonymity, more anonymity metrics could also be explored. Lastly, especially in the future as IPv6 adoption increases within the Tor network, it would be interesting to test the latency using not only IPv4 but also IPv6. 

\chapter{Conclusions}
The results show that when qTox was connected via vanilla Tor, the median latency was 517 ms higher than unmodified qTox for live video calls. When qTox's Tor connection was limited to only using relays within Sweden or Germany, the median relative latency was reduced by about 42 \% to 300 ms for Sweden and by about 39 \% to 317 ms for Germany. Vanilla Tor experienced more frequent and higher latency spikes than the other scenarios, however all scenarios using Tor experienced occasional latency spikes over 900 ms relative to when Tor was not used.
...
We used the following torrc file in order to limit the Tor relays to Sweden. For limiting relays to Germany, \textbf{SE} should be replaced with \textbf{DE}.
\\
\\
\noindent\fbox{
    \parbox{\textwidth}{
...
One sample test for each scenario is shown here. The y-axis for all graphs show latencies between 0 ms and 1000 ms. All measured delays can be found on GitHub \parencite{thesisGithub}.
